...
type FamilyAgendaEntry = AgendaEntry & {
  type: "event" | "bill";
};

type FamilyAgendaSchedule = Record<string, FamilyAgendaEntry[]>;
...
export default function CalendarScreen() {
  ...
  /* ---------- state ---------- */
  const [items, setItems] = useState<FamilyAgendaSchedule>({});
  ...
  const saveEntry = () => {
    ...
    setItems((prev) => {
      ...
      const newCalendarThing: FamilyAgendaEntry = {
        name: title.trim(),
        type,
        height: 50,
        day: key,
    ...
    return (
      <View style={[styles.container, { backgroundColor: colors.bg }]}>
        <Agenda
          ...
          renderItem={(item) => {
            const familyItem = item as FamilyAgendaEntry;
